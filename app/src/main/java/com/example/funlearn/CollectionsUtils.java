package com.example.funlearn;

public class CollectionsUtils {

    public static final String[][] fruits = {
            {"pomme", "la pomme"},
            {"banane", "la banane"},
            {"cerise", "le cerise"},
            {"abricot", "l' abricot"},
            {"ananas", "un ananas"},
            {"citron", "le citron"},
            {"fraise", "la fraise"},
            {"framboise", "la framboise"},
            {"orange", "une orange"},
            {"poire", "la poire"},
            {"pasteque", "la pastèque"},
            {"raisin", "le raisin"}
    };


    public static final String[][] animals = {
            {"serpent", "le serpent"},
            {"elephant", "un éléphant"},
            {"chevre", "la chèvre"},
            {"poisson", "le poisson"},
            {"chameau", "le chameau"},
            {"grenouille", "la grenouille"},
            {"girafe", "la girafe"},
            {"ours", "un ours"},
            {"herisson", "le hérisson"},
            {"hibou", "le hibou"},
            {"cerf", "le cerf"},
            {"loup", "le loup"},
            {"oiseau", "un oiseau"},
            {"kangourou", "le kangourou"},
            {"ecureuil", "un écureuil"},
            {"renard", "le renard"},
            {"cheval", "le cheval"},
            {"poule", "la poule"},
            {"coq", "le coq"},
            {"canard", "le canard"},
            {"cochon", "le cochon"},
            {"mouton", "le mouton"},
            {"vache", "la vache"},
            {"perroquet", "le perroquet"},
            {"souris", "le souris"},
            {"tortue", "la tortue"},
            {"lapin", "le lapin"},
            {"chat", "le chat"},
            {"chien", "le chien"}
    };

    public static final String[][] legumes = {
            {"betterave", "la betterave"},
            {"champignons", "les champignons"},
            {"haricots", "les haricots"},
            {"poivron", "le poivron"},
            {"oignon", "l' oignon"},
            {"carotte", "la carotte"},
            {"courgette", "la courgette"},
            {"aubergine", "une aubergine"},
            {"concombre", "la concombre"},
            {"pommedeterre", "la pomme de terre"},
            {"tomate", "la tomate"},
            {"epinards", "les epinards"},
            {"brocoli", "le brocoli"},
            {"mais", "le mais"}
    };


    public static final String[][] courses = {
            {"yogurt", "le yogurt"},
            {"riz", "le riz"},
            {"pain", "le pain"},
            {"oeuf", "un oeuf"},
            {"papiertoilette", "le papier toilette"},
            {"glace", "la glace"},
            {"biscuit", "le biscuit"},
            {"biscotte", "la biscotte"},
            {"spaghetti", "le spaghetti"},
            {"jusdorange", "le jus d' orange"},
            {"miel", "le miel"},
            {"lait", "le lait"}
    };

    public static final String[][] visage = {
            {"moustache", "le moustache"},
            {"barbe", "la barbe"},
            {"dents", "les dents"},
            {"sourcil", "le sourcil"},
            {"langue", "la langue"},
            {"menton", "le menton"},
            {"levre", "la lèvre"},
            {"front", "le front"},
            {"joue", "la joue"},
            {"cheveux", "les cheveux"},
            {"oreille", "l' oreille"},
            {"nez", "le nez"},
            {"bouche", "la bouche"},
            {"yeux", "les yeux"}
    };

    public static final String[][] vetements = {
            {"gants", "les gants"},
            {"peignoir", "le peignoir"},
            {"echarpe", "une écharpe"},
            {"chapeau", "le chapeau"},
            {"bonnet", "le bonnet"},
            {"casquette", "la casquette"},
            {"chaussettes", "les chaussettes"},
            {"chaussures", "les chaussures"},
            {"robe", "la robe"},
            {"blouse", "la blouse"},
            {"pantalon", "le pantalon"},
            {"shorts", "les shorts"},
            {"chemise", "la chemise"},
            {"jupe", "la jupe"}
    };

    public static final String[][] corps = {
            {"tete", "la tête"},
            {"poitrine", "la poitrine"},
            {"fesses", "les fesses"},
            {"epaule", "l' epaule"},
            {"orteil", "un orteil"},
            {"genou", "le genou"},
            {"ongle", "un ongle"},
            {"pied", "le pied"},
            {"cou", "le cou"},
            {"dos", "le dos"},
            {"jambe", "la jambe"},
            {"doigt", "le doigt"},
            {"main", "la main"},
            {"bras", "le bras"}
    };

    public static final String[][] couleurs = {
            {"noir", "noir"},
            {"blanc", "blanc"},
            {"jaune", "jaune"},
            {"rouge", "rouge"},
            {"bleu", "bleu"},
            {"vert", "vert"},
            {"gris", "gris"},
            {"rose", "rose"},
            {"violet", "violet"},
            {"marron", "marron"},
            {"orange", "orange"}
    };


}
